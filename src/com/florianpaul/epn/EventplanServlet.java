...
public class EventplanServlet extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		// The second basic ResourceType should be a name of the attendee. We want to know always, who will be at our events. 
		ResourceType name = new ResourceType();
		plan.getResourceTypes().add(name);

		name.setName("Name");
		name.setDescription("This is the field, where the name of the attendee can be stored.");
		...
		email.setDescription("This is the field, where the email of the attendee can be stored.");
		email.setDatatype(DataType.Text);
		
		
		// Now a special type of event is designed as a template for concrete events
		EventType bbq = new EventType();
		bbq.setName("Planning of any BBQs.");
		// We will use all the types of resources, that we created generally at the plan's level above.
		bbq.getResourceTypes().addAll(plan.getResourceTypes());
		
		// But, we won't ask for the email, that shouldn't be relevant for our little festival
		bbq.getResourceTypes().remove(email);
		
		
		// After we have created the template, it time to create a concrete festival
		Event birthday = new Event();
		birthday.setTitle("My Birthday Party with BBQ.");
		
		// Add this event to the drawer, so we can list it categorized by event-type.
		bbq.getEvents().add(birthday);
		
		
		// Now the attendee can visit my software and answer the relevant information so that i can plan my birthday party.
		// First person is peter
		Attendee peter = new Attendee();

		// Peter can choose any drinks of a list (because it's of type DataType.MultipleSelection
		peter.getResourceItems().add(water);
		peter.getResourceItems().add(beer);
		
		// We removed the email from the List of ResourceTypes, so peter is not asked for his email by the software.
		// Peter must set his name. The software creates a new ResourceItem that is associated with peter.
		ResourceItem petersName = new ResourceItem();
		petersName.setText("Peter");
		peter.getResourceItems().add(petersName);
		
		// After peter has given his data to the software, he is added to my concrete birthday event.
